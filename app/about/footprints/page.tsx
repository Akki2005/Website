    ...
    <div className="min-h-screen bg-[#FFF9E6] py-8 px-4">
      <div className="container mx-auto pb-8">
        ...
        <div className="bg-black py-6 mb-12">
          ...
          <h1 className="text-3xl font-bold text-center text-white">Our Footprints</h1>
        </div>
        <div className="max-w-4xl mx-auto relative">
          <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-[#B22222] transform -translate-x-1/2"></div>
          {events.map((event, index) => (
            <div key={event.year} className="mb-16 flex justify-between items-center">
              {index % 2 === 0 ? (
                <>
                  <TimelineContent event={event} index={index} side="left" />
                  <TimelineIcon event={event} index={index} setSelectedEventIndex={setSelectedEventIndex} />
        ...
        <AnimatePresence>
          {selectedEventIndex !== null && (
            ...
            >
              <Card className="bg-white border-2 border-[#B22222] max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
                <CardHeader>
                ...
                <CardContent>
                  <p className="text-[#4A2C2A]">{events[selectedEventIndex].description}</p>
                </CardContent>
              </Card>
            </motion.div>
          )}
...
function TimelineContent({ event, index, side }: { event: TimelineEvent; index: number; side: "left" | "right" }) {
  return (
    <motion.div
    ...
    >
      ...
      <h4 className="text-lg font-semibold text-[#4A2C2A] mb-2">{event.title}</h4>
...
}: { event: TimelineEvent; index: number; setSelectedEventIndex: (index: number) => void }) {
  return (
